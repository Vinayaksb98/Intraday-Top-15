...
## v5 Best / Decision-First Layout

The main scanner output is arranged in this order:
Symbol → Final Signal → Score → Grade → Confidence → Current Price →
BUY TRIGGER → Breakout Confirmed → BOP → RVOL → Stop Loss →
Target 1 → Target 2 → RR T1 → RR T2 → Resistance → supporting scores.

`BUY TRIGGER` is the breakout/entry trigger to monitor; it is not a guaranteed price.
The scanner uses `🟢 BUY`, `🟡 WAIT`, and `🔴 AVOID` to separate actionable signals
from strong pre-breakout watchlist setups.
