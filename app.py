...
st.title("📈 BOP TOP 15 — NSE / NIFTY 500")
...
def get_nifty500_universe():
    """Download the current Nifty 500 constituent list from NSE Indices."""
    urls=[
    ...
    for url in urls:
        try:
            ...
            if r.ok and len(r.content)>1000:
                ...
                symcol=next((c for c in d.columns if str(c).strip().upper()=="SYMBOL"),None)
                if symcol:
                    syms=sorted(set(
                        d[symcol].astype(str).str.strip().str.upper()
...
def get_market_caps(symbols):
    """
    Fetch current market capitalization from Yahoo Finance.
    Returns a DataFrame with NSE symbol and market cap in ₹ crore.

    This is cached for 6 hours so a scan does not repeatedly request
    fundamentals for the same Nifty 500 constituents.
    """
    ...
    def one(sym):
        try:
            t=yf.Ticker(sym+".NS")
            # fast_info is substantially lighter than a full ticker.info request.
            fi=t.fast_info
            mc=fi.get("market_cap", None)
            if mc is None:
                # Fallback for Yahoo versions where fast_info does not expose market_cap.
                info=t.info
                mc=info.get("marketCap", None)
            if mc is None or not np.isfinite(float(mc)):
                return sym, np.nan
            return sym, float(mc)/1e7  # ₹ -> ₹ crore
        except Exception:
            return sym, np.nan

    out=[]
    # Moderate concurrency balances speed against free-data rate limits.
    ...
    return pd.DataFrame(out,columns=["Stock","MarketCapCr"]).sort_values("Stock").reset_index(drop=True)


@st.cache_data(ttl=86400, show_spinner=False)
def get_nse_universe():
    """Legacy automatic NSE equity universe."""
    urls=[
...
with st.sidebar:
    ...
    universe=st.selectbox("Stock universe",[
        "Nifty 500 + Market Cap ≥ ₹10,000 Cr",
        "Nifty 500 (all constituents)",
        "All NSE Equity (automatic)",
        "NSE liquid scan (faster)",
        "My 7 reference stocks"
    ])
    min_market_cap=st.number_input(
        "Minimum Market Cap (₹ Cr)", min_value=0, value=10000, step=500
    )
    ...
    st.caption("Nifty 500 mode uses the current Nifty 500 constituent list from NSE Indices; the ₹10,000 Cr filter uses Yahoo Finance market-cap data.")
...
if "market_cap_map" not in st.session_state: st.session_state.market_cap_map={}

if st.button("🔄 Scan / Refresh Top 15"):
    market_cap_map={}
    if universe=="My 7 reference stocks":
        symbols=["VOLTAMP","EBGNG","EXICOM","CPPLUS","WELCORP","GVT&D","PARAS"]
    elif universe in ["Nifty 500 + Market Cap ≥ ₹10,000 Cr","Nifty 500 (all constituents)"]:
        symbols=get_nifty500_universe()
        if not symbols:
            st.error(
                "Could not download the current Nifty 500 constituent list from NSE Indices. "
                "Try again later. No substitute universe is used, so the scan will not silently "
                "scan the wrong stocks."
            )
            symbols=[]
        if symbols and universe=="Nifty 500 + Market Cap ≥ ₹10,000 Cr":
            with st.spinner("Getting current market capitalization for Nifty 500 constituents..."):
                mc=get_market_caps(symbols)
            mc=mc.dropna(subset=["MarketCapCr"])
            mc=mc[mc["MarketCapCr"]>=min_market_cap].copy()
            market_cap_map=dict(zip(mc["Stock"],mc["MarketCapCr"]))
            symbols=mc["Stock"].tolist()
            st.success(
                f"Nifty 500 filter: {len(symbols)} stocks have market cap ≥ "
                f"₹{min_market_cap:,.0f} Cr."
            )
        elif symbols:
            with st.spinner("Getting market capitalization for Nifty 500 constituents..."):
                mc=get_market_caps(symbols)
            market_cap_map=dict(zip(mc["Stock"],mc["MarketCapCr"]))
    else:
    ...
    st.session_state.market_cap_map=market_cap_map
            ...
            for sym in batch:
                ...
                try:
                    ...
                    rec={"Stock":sym,"Date":x.Date.date(),"Price":x.Close,"BOP":x.BOP,
                         ...
                         "MarketCapCr":st.session_state.get("market_cap_map",{}).get(sym,np.nan)}
