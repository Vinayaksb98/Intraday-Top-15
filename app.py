...
def decision_first_layout(df):
    """Trading-oriented column order and final signal."""
    df = df.copy()

    # Normalize legacy names.
    if "Current Price" in df.columns and "Close" not in df.columns:
        df = df.rename(columns={"Current Price": "Close"})

    # A clear BUY TRIGGER: breakout resistance when available.
    ...
    # Derive a conservative final signal.
    if "Final Signal" not in df.columns:
        def make_signal(r):
            try:
                ...
                score = 0
            breakout = str(r.get("Breakout Confirmed", "")).upper() == "YES"
            try:
                bop_ok = float(r.get("BOP", 0)) > 0
            except Exception:
                bop_ok = True
            try:
                rvol_ok = float(r.get("RVOL", 0)) >= 1.20
            except Exception:
                rvol_ok = True
            try:
                rr_ok = float(r.get("RR T2", 0)) >= 2.0
            except Exception:
                rr_ok = True

            if score >= 80 and breakout and bop_ok and rvol_ok and rr_ok:
                return "🟢 BUY"
            if score >= 80:
                return "🟡 WAIT"
            return "🔴 AVOID"
        df["Final Signal"] = df.apply(make_signal, axis=1)
