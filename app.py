...
def score(x):
    ...
    sig="PRE-BREAKOUT A" if s>=80 else "PRE-BREAKOUT B" if s>=70 else "WATCH" if s>=60 else "IGNORE"
    return {
        "Score":min(s,100),"Signal":sig,"Close":r.Close,"Entry Low":r.Close,
        "Entry High":entry_hi,"Stop Loss":stop,"Target 1":t1,"Target 2":t2,
        "RR T1":(t1-r.Close)/risk_abs,"RR T2":(t2-r.Close)/risk_abs,
