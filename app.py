...
st.caption("Pre-BOP early-entry scanner • Nifty 500 • Market Cap ≥ ₹10,000 Cr • Free daily-data research mode")
...
def prepare(df):
    ...
    # Pre-BOP features: identify stocks preparing for a breakout,
    # rather than stocks that have already broken out.
...
def score(d):
    x=d.iloc[-1]
    bb_threshold=d["BB_WIDTH"].rolling(20).quantile(0.40).iloc[-1]

    checks=[
      ("Price 0–3% below previous 20D high",
       pd.notna(x.DistanceToBO) and 0 <= x.DistanceToBO <= 3,2),
      ("EMA9 > EMA21",
       pd.notna(x.EMA9) and pd.notna(x.EMA21) and x.EMA9>x.EMA21,1),
      ("EMA20 > EMA50",
       pd.notna(x.EMA20) and pd.notna(x.EMA50) and x.EMA20>x.EMA50,1),
      ("RSI 52–65 and rising",
       pd.notna(x.RSI) and 52<=x.RSI<=65 and pd.notna(x.RSI_Slope) and x.RSI_Slope>0,2),
      ("MACD histogram improving",
       pd.notna(x.MACD_Hist_Slope) and x.MACD_Hist_Slope>0,1),
      ("RVOL >= 1.2x",
       pd.notna(x.RVOL) and x.RVOL>=1.2,1),
      ("ADX >= 18 and rising",
       pd.notna(x.ADX) and x.ADX>=18 and pd.notna(x.ADX_Slope) and x.ADX_Slope>0,1),
      ("Bollinger volatility compressed",
       pd.notna(x.BB_WIDTH) and pd.notna(bb_threshold) and x.BB_WIDTH<=bb_threshold,1)
    ]
    total=sum(p for _,ok,p in checks if ok)
    signal="🟢 BUY" if total>=8 else "🟡 WATCH" if total>=6 else "⚪ AVOID"
...
with st.sidebar:
    ...
    min_market_cap = st.number_input("Minimum Market Cap (₹ Cr)", min_value=0, value=10000, step=1000)
    st.caption("Target universe: Nifty 500 companies meeting the market-cap threshold.")
