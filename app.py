...
def render_decision_first_table(df):
    """Create and display the exact decision-first scanner layout."""
    df = df.copy()

    # Normalize common names.
    rename_map = {}
    if "Current Price" in df.columns and "Close" not in df.columns:
        rename_map["Current Price"] = "Close"
    if "Breakout" in df.columns and "Breakout Confirmed" not in df.columns:
        rename_map["Breakout"] = "Breakout Confirmed"
    if rename_map:
        df = df.rename(columns=rename_map)

    # Ensure required visible fields exist.
    if "BUY TRIGGER" not in df.columns:
        if "Resistance" in df.columns:
            df["BUY TRIGGER"] = df["Resistance"]
        elif "Entry High" in df.columns:
            df["BUY TRIGGER"] = df["Entry High"]
        elif "Close" in df.columns:
            df["BUY TRIGGER"] = df["Close"]

    if "Final Signal" not in df.columns:
        def final_signal(row):
            try: score = float(row.get("Score", 0))
            except: score = 0
            br = str(row.get("Breakout Confirmed", "")).upper() == "YES"
            try: bop = float(row.get("BOP", 0)) > 0
            except: bop = True
            try: rvol = float(row.get("RVOL", 0)) >= 1.20
            except: rvol = True
            try: rr2 = float(row.get("RR T2", 0)) >= 2.0
            except: rr2 = True
            if score >= 80 and br and bop and rvol and rr2:
                return "🟢 BUY"
            if score >= 80:
                return "🟡 WAIT"
            return "🔴 AVOID"
    ...
    preferred = [
        "Symbol", "Final Signal", "Score", "Grade", "Confidence",
        "Close", "BUY TRIGGER", "Breakout Confirmed", "BOP", "RVOL",
        "Stop Loss", "Target 1", "Target 2", "RR T1", "RR T2",
        "Resistance", "Distance to Resistance",
        "Trend", "Momentum", "Volume", "Breakout Setup", "Relative Strength"
    ]
    cols = [c for c in preferred if c in df.columns]
    cols += [c for c in df.columns if c not in cols]
    df = df[cols]

    st.subheader("📊 Decision-First Trade Scanner")
    st.caption("BUY requires a confirmed breakout plus the configured confirmation filters. WAIT means a strong setup is not yet confirmed.")

    # Explicit column configuration makes the requested fields visible in the exact order.
    config = {}
    for c in df.columns:
        if c == "Final Signal":
            config[c] = st.column_config.TextColumn("Final Signal", help="🟢 BUY / 🟡 WAIT / 🔴 AVOID")
        elif c == "BUY TRIGGER":
            config[c] = st.column_config.NumberColumn("BUY TRIGGER", format="₹%.2f")
        elif c == "Close":
            config[c] = st.column_config.NumberColumn("Current Price", format="₹%.2f")
        elif c == "Breakout Confirmed":
            config[c] = st.column_config.TextColumn("Breakout Confirmed", help="YES/NO")
        elif c == "BOP":
            config[c] = st.column_config.NumberColumn("BOP", format="%.4f")
        elif c == "RVOL":
            config[c] = st.column_config.NumberColumn("RVOL", format="%.2f")
        elif c in ["Stop Loss", "Target 1", "Target 2", "Resistance"]:
            config[c] = st.column_config.NumberColumn(c, format="₹%.2f")
        elif c in ["RR T1", "RR T2"]:
            config[c] = st.column_config.NumberColumn(c, format="%.2f")
    render_decision_first_table(df)
    return df
